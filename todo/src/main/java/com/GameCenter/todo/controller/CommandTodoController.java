...
public class CommandTodoController {
    ...
    public ResponseEntity todoAdd(@RequestBody TodoListDTO DTO){
        return new ResponseEntity(new Message(HttpStatus.CREATED.value(),"생성 완료",todoService.TodoListAdd(DTO),null),HttpStatus.CREATED);
    }

    @PutMapping("/Update/{id}")
    public ResponseEntity todoUpdate(@PathVariable int id, @RequestBody TodoListDTO DTO){
        return new ResponseEntity(new Message(HttpStatus.OK.value(),"수정 완료",todoService.TodoListUpdate(id,DTO),null),HttpStatus.OK);
    }
